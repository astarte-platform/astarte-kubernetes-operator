...
var _ = Describe("Utils functions testing", Ordered, Serial, func() {
	...
	AfterEach(func() {
		...
		for _, a := range astartes.Items {
			astarte := a // Capture loop variable
			if err := k8sClient.Delete(context.Background(), &astarte); err != nil && !apierrors.IsNotFound(err) {
				Expect(err).ToNot(HaveOccurred())
			}
		}
		// Wait for all Astarte instances to be deleted
		Eventually(func() int {
			astarteList := &apiv2alpha1.AstarteList{}
			if err := k8sClient.List(context.Background(), astarteList, &client.ListOptions{Namespace: CustomAstarteNamespace}); err != nil {
				return -1
			}
			return len(astarteList.Items)
		}, Timeout, Interval).Should(Equal(0))

		// Wait for all secrets to be deleted		// Clean up secrets, deployments, etc.
		secrets := &v1.SecretList{}
		Expect(k8sClient.List(context.Background(), secrets, &client.ListOptions{Namespace: CustomAstarteNamespace})).To(Succeed())
		for _, s := range secrets.Items {
			secret := s // Capture loop variable
			if err := k8sClient.Delete(context.Background(), &secret); err != nil && !apierrors.IsNotFound(err) {
				Expect(err).ToNot(HaveOccurred())
			}
		}

		// Wait for all secrets to be deleted
		Eventually(func() int {
			secretList := &v1.SecretList{}
			if err := k8sClient.List(context.Background(), secretList, &client.ListOptions{Namespace: CustomAstarteNamespace}); err != nil {
				return -1
			}
		...
		Expect(k8sClient.List(context.Background(), serviceAccounts, &client.ListOptions{Namespace: CustomAstarteNamespace})).To(Succeed())
		...
		Eventually(func() int {
			...
			if err := k8sClient.List(context.Background(), saList, &client.ListOptions{Namespace: CustomAstarteNamespace}); err != nil {
				return -1
			}
		...
		Expect(k8sClient.List(context.Background(), roles, &client.ListOptions{Namespace: CustomAstarteNamespace})).To(Succeed())
		...
		Eventually(func() int {
			...
			if err := k8sClient.List(context.Background(), roleList, &client.ListOptions{Namespace: CustomAstarteNamespace}); err != nil {
				return -1
			}
		...
		Expect(k8sClient.List(context.Background(), roleBindings, &client.ListOptions{Namespace: CustomAstarteNamespace})).To(Succeed())
		...
		Eventually(func() int {
			...
			if err := k8sClient.List(context.Background(), rbList, &client.ListOptions{Namespace: CustomAstarteNamespace}); err != nil {
				return -1
			}
			return len(rbList.Items)
		}, Timeout, Interval).Should(Equal(0))

		// Final check - ensure all Astarte instances are gone
		Eventually(func() int {
