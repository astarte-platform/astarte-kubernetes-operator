...
const (
	...
	rabbitmqClusterOperatorVersion = "v2.16.0"
	rabbitmqClusterOperatorURL     = "https://github.com/rabbitmq/cluster-operator/releases/download/%s/cluster-operator.yml"
...
func InstallRabbitMQClusterOperator() error {
	...
	manifestPath := fmt.Sprintf("%s/test/manifests/rabbitmq-cluster.yaml", prj)
	...
	// Ensure the Astarte namespace exists
...
func InstallScyllaOperator() error {
	...
	// Sleep for 15 seconds to allow the manifest to be applied
	// TODO: Use a more robust waiting mechanism
	time.Sleep(15 * time.Second)
